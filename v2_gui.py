...
from ui_components import WorkspaceDisplay, V2ControlPanel, AdvancedToolTip
...
class V2Calculator(ExecuteModeMixin):
    ...
    def create_left_panel(self):
        ...
        AdvancedToolTip(
            self.calc_type,
            self.tooltips.get("HesaplamaTuru", "Bir hesaplama türü seçin."),
        )
        ...
        AdvancedToolTip(
            self.calculation,
            self.tooltips.get("HesaplamaSecimi", "Bir hesaplama seçin."),
        )
        ...
        self.calculate_btn = ttk.Button(
            self.left_frame, text="🔷 HESAPLA", command=self.calculate
        )
        self.calculate_btn.pack(fill="x", pady=10)
        AdvancedToolTip(
            self.calculate_btn,
            self.tooltips.get("HesaplaButonu", "Seçili hesaplamayı gerçekleştir."),
        )

        # Clear button (NEW V2)
        self.clear_params_btn = ttk.Button(
            self.left_frame, text="🗑️ TEMİZLE", command=self.clear_parameters
        )
    ...
    def create_bottom_panel(self):
        ...
        # Model URL
        ttk.Label(model_frame, text="Model URL:", style="Calc.TLabel").pack(
            side="left", padx=(0, 5)
        )
        ...
        # Model selection
        ttk.Label(model_frame, text="Model:", style="Calc.TLabel").pack(
            side="left", padx=(0, 5)
        )
        self.model_selection_combo = ttk.Combobox(
            model_frame, width=20, state="readonly"
        )
        self.model_selection_combo.pack(side="left", padx=(0, 10))

        # Refresh models button
        self.refresh_models_btn = ttk.Button(
            model_frame, text="🔄", width=3, command=self.refresh_model_list
        )
        ...
        self.test_connection_btn = ttk.Button(
            model_frame, text="🔗", width=3, command=self.test_model_connection
        )
    ...
    def initialize_calculation_types(self):
        ...
        self.calc_types = {
            ...
            "Tornalama Hesaplamaları": {
                key: {} for key in ec.turning_definitions.keys()
            },
            "Frezeleme Hesaplamaları": {
                key: {} for key in ec.milling_definitions.keys()
            },
        }
    ...
    def update_input_fields(self, event=None):
        ...
        if (
            calc_category == "Malzeme Hesaplamaları"
            and selected_calculation == "Kütle Hesabı"
        ):
        ...
        if calc_category_internal:
            try:
                ...
                for param_info in param_info_list:
                    ...
                    label_text = (
                        f"{param_info['display_text_turkish']} [{param_info['unit']}]:"
                    )
                    ...
                    tooltip_text = self.tooltips.get(
                        tooltip_key,
                        self.tooltips.get(
                            param_info["display_text_turkish"],
                            f"{param_info['display_text_turkish']} girin.",
                        ),
                    )
    ...
    def _update_material_params(self, event=None):
        ...
        ttk.Label(density_frame, text="Yoğunluk [g/cm³]:", style="Calc.TLabel").pack(
            side="left", padx=(0, 5)
        )
        ...
        AdvancedToolTip(
            density_entry,
            self.tooltips.get("Yoğunluk", "Malzemenin yoğunluğunu girin."),
        )

        # Add length field
        length_frame = ttk.Frame(self.params_container, style="Calc.TFrame")
        length_frame.pack(fill="x", pady=2, padx=5)
        ttk.Label(length_frame, text="Uzunluk [mm]:", style="Calc.TLabel").pack(
            side="left", padx=(0, 5)
        )
        ...
        AdvancedToolTip(
            length_entry, self.tooltips.get("Uzunluk", "Şeklin uzunluğunu girin.")
        )
        ...
        for param_name in shape_params:
            ...
            tooltip_text = self.tooltips.get(
                param_name, f"{param_name} değerini girin."
            )
            AdvancedToolTip(entry, tooltip_text)
        ...
        try:
            ...
            for param_key, field_widget in self.input_fields.items():
                ...
                try:
                    ...
                    messagebox.showerror(
                        "Hata",
                        f"'{param_key}' için geçersiz sayısal değer: {value_str}",
                    )
                    return
            ...
            self.update_status_bar(
                f"Hesaplama eklendi: {calculation_result['value']} {calculation_result['unit']}"
            )
        ...
        mass_params = prepare_material_mass_arguments(
            ec,
            {
                ...
                **{
                    ...
                    if k not in ["Şekil", "density", "length"]
                },
            },
            [],
        )
    ...
    def handle_edit_calculation(self, calc_id: str, updated_calculation):
        ...
        if success:
            # Add updated calculation back to workspace
            new_calc_id = self.workspace_manager.add_calculation(
                calc_type=updated_calculation.calc_type,
                calc_name=updated_calculation.calculation_name,
                params=updated_calculation.parameters,
                result=updated_calculation.result,
                unit=updated_calculation.unit,
            )

            # Update display
            self.workspace_display.remove_calculation_card(calc_id)
            new_calculation = self.workspace_manager.get_calculation(new_calc_id)
            self.workspace_display.add_calculation_card(new_calculation)

            self.update_status_bar("Hesaplama güncellendi")
        else:
            messagebox.showerror("Hata", "Hesaplama güncellenemedi")
    ...
    def handle_compare_calculations(self):
        ...
        if len(calculations) < 2:
            messagebox.showinfo(
                "Bilgi", "Karşılaştırma için en az 2 hesaplama gerekli."
            )
            return
    ...
    def export_workspace(self):
        ...
        try:
            ...
            if file_path:
                ...
                messagebox.showinfo(
                    "Başarılı", f"Çalışma alanı dışa aktarıldı:\n{file_path}"
                )
