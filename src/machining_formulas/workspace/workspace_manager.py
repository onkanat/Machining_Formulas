...
Bu dosya, mevcut kod tabanındaki `workspace_manager.py` ile aynıdır.
Paket içi importlar için `src/` altına kopyalanmıştır.
"""
...
class CalculationEntry:
    ...
    model_comments: List[str] = field(default_factory=list)
...
class Workspace:
    ...
    def get_full_context(self) -> str:
        ...
        context = f"Çalışma Alanı İçeriği (Toplam {len(self.calculations)} hesaplama):\n\n"
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "Workspace":
        ...
        if "calculations" in data:
            workspace.calculations = [
                CalculationEntry.from_dict(calc_data) for calc_data in data["calculations"]
            ]
...
class WorkspaceManager:
    ...
    def add_calculation(
        ...
        return self.workspace.add_calculation(calc_type, calc_name, params, result, unit)
