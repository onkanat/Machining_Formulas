...
def _resolve_density(
    ...
    material_key = next(
        (key for key in ("material", "malzeme", "material_name") if key in args),
        None,
    )
    if material_key:
...
def _ensure_radius_argument(
    ...
    for key in _DIAMETER_KEYS:
        if key in args:
            ...
            radius_val = _resolve_radius_from_alias(alias_value, inferred_diameter)
            args["radius"] = radius_val
            return


def _density_from_catalog(calculator: EngineeringCalculator, material: str) -> float:
    try:
...
def _reconcile_existing_radius(args: Dict[str, Any], inferred_diameter: Optional[float]) -> None:
...
def _resolve_radius_from_alias(alias_value: float, inferred_diameter: Optional[float]) -> float:
